...
// Catálogo de benefícios do programa de indicação
export const referralBenefitCatalog = pgTable("referral_benefit_catalog", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  name: text("name").notNull(),
  description: text("description"),
  type: varchar("type").notNull(), // "B1" | "B2"
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const insertReferralBenefitCatalogSchema = createInsertSchema(
  referralBenefitCatalog,
).omit({ id: true, createdAt: true });
export type InsertReferralBenefitCatalog = z.infer<
  typeof insertReferralBenefitCatalogSchema
>;
export type ReferralBenefitCatalog =
  typeof referralBenefitCatalog.$inferSelect;

// Histórico de entregas de benefícios
export const referralBenefitDeliveries = pgTable("referral_benefit_deliveries", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  referrerId: varchar("referrer_id")
    .references(() => clients.id, { onDelete: "cascade" })
    .notNull(),
  benefitCatalogId: varchar("benefit_catalog_id")
    .references(() => referralBenefitCatalog.id)
    .notNull(),
...
export const insertReferralBenefitDeliverySchema = createInsertSchema(
  referralBenefitDeliveries,
).omit({ id: true, deliveredAt: true });
export type InsertReferralBenefitDelivery = z.infer<
  typeof insertReferralBenefitDeliverySchema
>;
...
export const restaurantOrderAuditLog = pgTable(
  "restaurant_order_audit_log",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
...
export const insertRestaurantOrderAuditLogSchema = createInsertSchema(
  restaurantOrderAuditLog,
).omit({
  id: true,
  createdAt: true,
});

export const insertRestaurantOrderPaymentSchema = createInsertSchema(
...
export type InsertRestaurantOrderAuditLog = z.infer<
  typeof insertRestaurantOrderAuditLogSchema
>;
export type RestaurantOrderAuditLog = typeof restaurantOrderAuditLog.$inferSelect;
export type InsertRestaurantOrderPayment = z.infer<
...
export const whatsappMessages = pgTable("whatsapp_messages", {
  ...
  status: text("status", { enum: ["pending", "sent", "delivered", "read", "failed", "deleted"] }),
  ...
  replyToContentSnapshot: text("reply_to_content_snapshot"),
  replyToTypeSnapshot: text("reply_to_type_snapshot"),
  replyToDirectionSnapshot: text("reply_to_direction_snapshot", {
...
export const waSavedStickers = pgTable(
  "wa_saved_stickers",
  {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
    mediaId: varchar("media_id").notNull().references(() => whatsappMedia.id, { onDelete: "cascade" }),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (t) => ({ userMediaUnique: unique().on(t.userId, t.mediaId) }),
);
...
export const waQuickReplies = pgTable(
  ...
  (t) => ({ userTitleUnique: unique().on(t.userId, t.title) }),
);
