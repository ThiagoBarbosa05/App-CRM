...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Cashback Reports - últimos 30 dias
  app.get("/api/cashback-reports/30-days", async (req, res) => {
    try {
      ...
      const allTransactions = await storage.getCashbackTransactions(userId, userRole);
      const transactionsLast30Days = allTransactions.filter(transaction =>
        new Date(transaction.cashback_transactions?.createdAt || transaction.createdAt) >= thirtyDaysAgo &&
        (transaction.cashback_transactions?.status || transaction.status) === "approved"
      );

      // Buscar resgates dos últimos 30 dias
      const allUsage = await storage.getAllCashbackUsage(userId, userRole);
      const usageLast30Days = allUsage.filter(usage =>
        new Date(usage.createdAt) >= thirtyDaysAgo
      );

      // Calcular totais
      const totalSales = salesLast30Days.length;
      const totalSalesValue = salesLast30Days.reduce((sum, sale) => 
        sum + parseFloat(sale.grossValue.toString()), 0
      );

      const totalCashbackGenerated = transactionsLast30Days.reduce((sum, transaction) => {
        const t = transaction.cashback_transactions || transaction;
        return sum + parseFloat(t.cashbackAmount || 0);
      }, 0);

      const totalCashbackRedeemed = usageLast30Days.reduce((sum, usage) =>
        sum + parseFloat(usage.usedAmount || 0), 0
      );

      const totalNetValue = salesLast30Days.reduce((sum, sale) =>
        sum + parseFloat(sale.netValue.toString()), 0
      );

      const totalCashbackUsedInSales = salesLast30Days.reduce((sum, sale) =>
        sum + parseFloat(sale.cashbackUsed.toString()), 0
      );
      ...
      res.json({
        ...
        totalCashbackUsedInSales,
        salesData: salesLast30Days,
        transactionsData: transactionsLast30Days,
        usageData: usageLast30Days
      });
    } catch (error) {
