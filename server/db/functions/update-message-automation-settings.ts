...
export const updateMessageAutomationSettingSchema = createInsertSchema(messageAutomationSettings, {
    sendTime: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Formato de hora inválido (HH:mm)").optional(),
    daysBefore: z.number().int().min(0, "Os dias anteriores devem ser um número não negativo").optional(),
}).partial().extend({
  id: z.string({ required_error: "ID is required to update a setting." }),
});

export type UpdateMessageAutomationSettingInput = z.infer<typeof updateMessageAutomationSettingSchema>;
