...
const BAR_STYLES = [
  {
    bar: "from-primary via-primary/80 to-primary/60",
    glow: "bg-primary/15",
    badge: "bg-primary/10 text-primary dark:bg-primary/20 border-primary/20",
  },
  {
    bar: "from-primary/90 via-primary/70 to-primary/50",
    glow: "bg-primary/10",
    badge: "bg-accent text-primary border-border",
  },
  {
    bar: "from-primary/70 via-primary/50 to-primary/30",
    glow: "bg-primary/8",
    badge: "bg-accent text-primary/80 border-border",
  },
  {
    bar: "from-slate-400 via-slate-300 to-slate-200",
    glow: "bg-slate-300/10",
    badge:
      "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-300 border-slate-200/50",
  },
  {
    bar: "from-slate-300 via-slate-200 to-slate-100",
    glow: "bg-slate-200/10",
    badge:
      "bg-white text-slate-500 dark:bg-slate-900 dark:text-slate-300 border-slate-100",
  },
];
...
export function ProductsStatistics({
  ...
  } = useQuery<{ quantityByProduct: any[] }>({
  ...
  const {
    ...
    isFetching: isFetchingWine,
  } = useQuery<{ revenueByType: any[] }>({
  ...
  const totalQty = filteredByType.reduce(
    (acc: number, p: any) => acc + parseFloat(p.totalQuantity || "0"),
    0,
  );
  const maxQty = Math.max(
    ...filteredByType.map((p: any) => parseFloat(p.totalQuantity || "0")),
    0,
  );
                      ...
                      className={cn(
                        ...
                        selectedType === t
                          ? cn(
                              ...
                              "ring-2 ring-offset-1 ring-current border-transparent",
                            )
                ...
                {isLoadingQty || isFetchingQty ? (
                  ...
                  <div className="space-y-2">
                    ...
                    {filteredByType.map((product: any) => {
                      ...
                      const barWidth =
                        maxQty > 0 ? Math.max((qty / maxQty) * 100, 4) : 0;
                      const typeStyle =
                        TYPE_BUTTON_STYLES[product.productType as WineType] ??
                        TYPE_BUTTON_STYLES["TODOS"];
