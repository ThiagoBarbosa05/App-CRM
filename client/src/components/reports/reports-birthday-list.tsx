...
export function ReportsBirthdayList({
  upcomingBirthdays,
}: ReportsBirthdayListProps) {
    ...
    <Card className="group border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 rounded-3xl overflow-hidden bg-white dark:bg-slate-900 border-t-4 border-t-amber-500">
      <CardHeader className="pb-4 border-b border-slate-50 dark:border-slate-800 bg-amber-50/30 dark:bg-amber-900/10">
        <div className="flex items-center justify-between gap-3">
          ...
          {sorted.length > 0 && (
            <div className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-bold px-2.5 py-1 rounded-full">
              {sorted.length}
            </div>
          )}
        </div>
              ...
              <div className="divide-y divide-slate-50 dark:divide-slate-800">
                {sorted.map((client, index) => (
                  <motion.div
                    ...
                    className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                  >
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      <div className="w-10 h-10 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-full flex items-center justify-center border border-amber-100 dark:border-amber-800/30 flex-shrink-0">
