...
export function OpenTableDialog({
  ...
  const { data: searchResults = [], isLoading: isSearching } = useQuery<ClientResult[]>({
    ...
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", `/api/restaurant-pdv/clients/search?q=${encodeURIComponent(debouncedQuery.trim())}`);
      return res.json();
    },
  ...
  function handleConfirm(skipClient: boolean) {
    onConfirm({
      tableNumber,
      peopleCount,
      clientId: skipClient ? null : (selectedClient?.id ?? null),
      clientName: skipClient ? null : (selectedClient?.name ?? null),
    });
  }

  const step1Valid =
    tableNumber && Number(tableNumber) > 0 && peopleCount && Number(peopleCount) > 0;
        ...
        {step === 1 ? (
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Busque um cliente existente ou cadastre um novo. Ou abra sem vínculo.
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="flex-col gap-2 sm:flex-row">
              <Button
                type="button"
                variant="ghost"
                className="order-2 sm:order-1 w-full sm:w-auto text-muted-foreground"
              ...
              >
                {isPending ? (
                  ...
                  selectedClient ? `Abrir com ${selectedClient.name.split(" ")[0]}` : "Abrir Mesa"
                )}
              </Button>
            </DialogFooter>
          </>
