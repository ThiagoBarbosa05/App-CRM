        ...
        <div className="container mx-auto p-6 space-y-6">
          ...
          {!isLoading && filteredGoals.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {formatCurrency(
                      filteredGoals.reduce((sum, goal) => sum + Number(goal.salesGoal), 0).toString()
                    )}
                  </div>
                  ...
                  <div className="text-2xl font-bold">
                    {formatCurrency(
                      filteredGoals.reduce((sum, goal) => {
                        ...
                        return sum + totalSalesAchieved;
                      }, 0).toString()
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Total vendas parciais
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">% de Cumprimento</CardTitle>
                  <BarChart3 className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">
                    {(() => {
                      const totalGoal = filteredGoals.reduce((sum, goal) => sum + Number(goal.salesGoal), 0);
                      const totalAchieved = filteredGoals.reduce((sum, goal) => {
                        const achieved = getTotalAchieved(goal.weeklyResults, 'salesAchieved');
                        return sum + achieved;
                      }, 0);
